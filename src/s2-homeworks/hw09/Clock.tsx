...
function Clock() {
    ...
    const start = () => {
        ...
        // сохранить ид таймера (c)
        let newDimerId  = setInterval(() => {
            setDate(new Date(restoreState('hw9-date', Date.now())))
        }, 1000)
        setTimerId(+newDimerId)
    ...
    let formatterMount = new Intl.DateTimeFormat("en", {
        month: "long"
    })
    ...
    const stringDate =  date.toLocaleDateString("ru-RU") || <br/> // при наведении
    ...
    const stringMonth = formatterMount.format(date) || <br/> // пишут студенты при наведении
